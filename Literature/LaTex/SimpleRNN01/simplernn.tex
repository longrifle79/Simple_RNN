...
% ==================== COLORS (consistent with your previous documents) ====================
\definecolor{headingRed}{RGB}{178, 34, 34}     % red for main headings
\definecolor{labelBlue}{RGB}{0, 102, 204}      % blue for sub-labels

% ==================== C++ IDE-STYLE SYNTAX HIGHLIGHTING ====================
\definecolor{codeKeyword}{RGB}{0, 0, 139}      % dark blue (keywords)
\definecolor{codeComment}{RGB}{0, 128, 0}      % green (comments)
\definecolor{codeString}{RGB}{163, 21, 21}     % red (strings)
\definecolor{codeNumber}{RGB}{175, 0, 175}     % magenta (numbers)
\definecolor{codeType}{RGB}{0, 128, 128}       % teal (types like VectorXd)
...
\lstdefinestyle{cppide}{
    ...
    tabsize=4,
    morekeywords={VectorXd, MatrixXd, std::random_device, std::mt19937, std::normal_distribution, std::function, std::tanh, Eigen},
    moretypes={VectorXd, MatrixXd}
}

\begin{document}

\title{SimpleRNN Class Implementation \\ \texttt{rnn.cpp}}
\author{Gary Hobson}
\date{}
\maketitle

\section*{Creating the SimpleRNN Class}

The full implementation of the \texttt{SimpleRNN} class is shown below with IDE-style syntax highlighting (blue keywords, green comments, red strings, teal types).

\begin{lstlisting}[style=cppide]
